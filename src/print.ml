...
and pp_type ppf ty = match ty with
  ...
  (* App *)
  | TyTyApp(abs, arg)       -> fprintf ppf "(%a %a)" pp_type abs pp_type arg
...
and pp_tyerr d ppf s = match s with
  | SensError (si1, si2)  -> fprintf ppf "EEE [%3d] Cannot satisfy constraint %a <= %a" d pp_si si1 pp_si si2
  | MoreGeneral(ty1, ty2) -> fprintf ppf "EEE [%3d] %a is not more general than %a"     d pp_type ty1 pp_type ty2
  | TypeInst(ty1, ty2)    -> fprintf ppf "EEE [%3d] Type %a is not instance of %a"      d pp_type ty1 pp_type ty2
  | TypeMismatch(ty1, ty2)-> fprintf ppf "EEE [%3d] Cannot unify %a with %a"        d pp_type ty1 pp_type ty2
  | CannotApply(ty1, ty2) -> fprintf ppf "EEE [%3d] Cannot apply %a to %a"    d pp_type ty1 pp_type ty2
  | OccursCheck(v, ty)    -> fprintf ppf "EEE [%3d] Cannot build infinite type %a = %a" d pp_vinfo v pp_type ty
...
and string_of_term_prim t = match t with
  ...
  | PrimTString s     -> ("\"" ^ s ^ "\"")
  | PrimTToken(i,ty)  -> "(Token #"^string_of_int i^")"
...
and pp_ttslst ttslst = 
  let pp ppf (tm,ty,si) = fprintf ppf "(%a :[%a] %a)" pp_term tm pp_si si pp_type ty in
  pp_list pp ttslst
...
and pp_term ppf t =
  ...
  | TmPVal  (_,tm) -> fprintf ppf "(PVAL (@[%a@]))" pp_term tm
  | TmUnPVal(_,tm) -> fprintf ppf "(UNPVAL (@[%a@]))" pp_term tm
...
let pp_to_string = asprintf

let kasprintf (k : string -> 'a) = kfprintf (fun _ -> k (Format.flush_str_formatter ())) str_formatter
